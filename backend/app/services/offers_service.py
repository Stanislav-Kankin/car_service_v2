...
class OffersService:
    ...
    async def update_offer(
        ...
        # КРИТИЧЕСКИЙ МОМЕНТ:
        #   exclude_unset → убирает неустановленные поля
        #   exclude_none  → дополнительно убирает поля со значением None
        #
        # Иначе status=None прилетает в БД и ломает NOT NULL constraint.
        data = data_in.model_dump(exclude_unset=True, exclude_none=True)
